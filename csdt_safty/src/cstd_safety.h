...
#define CSTD_safety_subsonic_leak 0   //亚音速泄露
#define CSTD_safety_supersonic_leak 1 //超音速泄露
...
#define radiant_heat_flux_pillar (double)270.0
	...
	//以下运算函数无错误返回0，出错返回<0
	//气体泄露模型计算
	EXPORT int CSTD_safety_gas_leak_calc(CSTD_safety_gas_leak_t *gas_leak);
	//液体泄露模型
	EXPORT int CSTD_safety_liquid_leak_calc(CSTD_safety_liquid_leak_t *liquid_leak);
	//固体火灾模型计算
	EXPORT int CSTD_safety_solid_fire_calc(CSTD_safety_solid_fire_t *solid_fire);
	//池火灾模型计算
	EXPORT int CSTD_safety_pool_fire_calc(CSTD_safety_pool_fire_t *pool_fire);
	//喷射火模型计算
	EXPORT int CSTD_safety_jet_fire_calc(CSTD_safety_jet_fire_t *jet_fire);
	//沸腾液体扩展蒸气爆炸模型计算
	EXPORT int CSTD_safety_BLEVE_fire_calc(CSTD_safety_BLEVE_fire_t *BLEVE_fire);
	//蒸汽云爆炸模型计算
	EXPORT int CSTD_safety_vcloud_explo_calc(CSTD_safety_vcloud_explo_t *vcloud_explosion);
	//凝聚相含能材料（爆炸物，如火药、炸药等）爆炸模型计算
	EXPORT int CSTD_safety_explosive_explo_calc(CSTD_safety_explosive_explo_t *explosive_explo);
	//盛装气体压力容器爆破模型计算
	EXPORT int CSTD_safety_gpress_vessel_explo_calc(CSTD_safety_gpress_vessel_explo_t *press_explo);
	//盛装液体压力容器爆破模型计算
	EXPORT int CSTD_safety_lpress_vessel_explo_calc(CSTD_safety_lpress_vessel_explo_t *press_explo);
	//盛装液化气体容器爆破模型计算
	EXPORT int CSTD_safety_lgpress_vessel_explo_calc(CSTD_safety_lgpress_vessel_explo_t *press_explo);
	//盛装饱和水容器爆破模型计算
	EXPORT int CSTD_safety_satwater_vessel_explo_calc(CSTD_safety_satwater_vessel_explo_t *press_explo);
    ...
    struct CSTD_point_2d
    {
        int x;
        int y;
    };
    typedef struct CSTD_point_2d CSTD_point_2d_t;
    //非重气云扩散模型,返回值不使用后必须用CSTD_safety_free函数释放
    //conce  气体浓度
    //stability  大气稳定度
    //step  计算坐标点的间隔距离，单位是米
    //emission_rate  泄露速度
    //height  气体高度
    //wind_speed  风速
    //len  计算出的坐标点个数
    //返回值：计算出的坐标点
    EXPORT CSTD_point_2d_t *CSTD_safety_Gaussian_plume_calc(double conce, int stability, double emission_rate, int height, int wind_speed, int* len);
	// 授权算法库的使用， file_name：key文件名
	//返回值：0 成功，-1 授权认证失败
	EXPORT int CSTD_authorize(const char *file_name);

    EXPORT void CSTD_safety_free( void *p);
#ifdef   __cplusplus
}
#endif
